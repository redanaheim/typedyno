...
export const process_message = async function (
    ...
    const command_results = await process_message_for_commands(
        message,
        client,
        pool,
    );
    ...
    if (command_results.did_find_command === true) {
        if (command_results.command_authorized === false) {
            if (is_string(command_results.not_authorized_reason)) {
                message.channel.send(
                    `You are not authorized to use that command. Reason: ${command_results.not_authorized_reason}`,
                );
            } else {
                message.channel.send(
        ...
        if (
            command_results.command_worked === true &&
            is_number(command_results.call_to_return_span_ms)
        ) {
            log(
                `Bot command "${
    ...
    if (
        ...
        command_results.did_find_command === false
    ) {
