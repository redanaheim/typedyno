  ...
  if (command_results.did_find_command === true) {
    if (command_results.command_authorized === false) {
      if (is_string(command_results.not_authorized_reason)) {
        ...
        message.channel.send("You are not authorized to use that command. This could be because of command permissions. This command might not allow you to view who is authorized.")
      }
    }
